...
class PositionTests(TestCase):
    ...
    def test_retrieve_all(self):
        response   = self.client.get(reverse('position-api-list'))
        data       = Position.objects.all()
        serializer = PositionSerializer(data, context={'request': None}, many=True)
    ...
    def test_valid_get_element(self):
        response = self.client.get(reverse('position-detail',kwargs={'pk': self.position_1.id}))
        data = Position.objects.get(pk=self.condicao_1.id)
    ...
    def test_invalid_get_element(self):
        response = self.client.get(reverse('position-detail',kwargs={'pk': 666}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    # Delete um elemento valido
    def test_valid_delete(self):
        response = self.client.delete(reverse('position-detail',kwargs={'pk': self.position_1.id}))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)

    # Delete um elemento valido
    def test_invalid_delete(self):
        response = self.client.delete(reverse('position-detail',kwargs={'pk': 666}))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
